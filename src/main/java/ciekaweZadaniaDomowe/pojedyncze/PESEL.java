...
public class PESEL {
    ...
    static boolean isValid(String pesel) throws Exception {
        String komunikat = "";
        if (pesel.length() != 11) {
            komunikat = "Niepoprawna długość numeru PESEL";
            throw new Exception(komunikat);
        }
        char[] peselarray = pesel.toCharArray();
        for (char c : peselarray) {
            if (c < 48 || c > 57) { // cyfry to znaki o wartości ASCII od 48 do 57.
                komunikat = "Nr PESEL powinien zawierać tylko liczby";
                throw new Exception(komunikat);
            }
        }
        ...
        String[] sex;
        if (peselarray[9] % 2 == 0) {
            sex = new String[]{"Kobieta", ", która urodzi się w dniu ", ", która urodziła się dnia ", " przyszła na świat nowa obywatelka"};
        } else {
            sex = new String[]{"Mężczyzna", ", który urodzi się w dniu ", ", który urodził się dnia ", " przyszedł na świat nowy obywatel"};
        }
